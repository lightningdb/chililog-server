...
{
    private static Log4JLogger _logger = Log4JLogger.getLogger(SslContextManager.class);
    ...
    {
        public static final SslContextManager INSTANCE = new SslContextManager();
    }
    ...
    private SslContextManager()
    {
        try
        {
            ...
            try
            {
                ...
                FileInputStream fin = new FileInputStream(AppProperties.getInstance().getManagementKeyStorePath());
                ks.load(fin, AppProperties.getInstance().getManagementKeyStorePassword().toCharArray());

                // Set up key manager factory to use our key store
                KeyManagerFactory kmf = KeyManagerFactory.getInstance(algorithm);
                kmf.init(ks, AppProperties.getInstance().getManagementKeyStoreKeyPassword().toCharArray());
            ...
            try
            {
                ...
                clientContext.init(null, SSLTrustManager.getInstance().getTrustManagers(), null);
            }
            catch (Exception e)
            {
