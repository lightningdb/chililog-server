...
{
    private static Log4JLogger _logger = Log4JLogger.getLogger(UiServerManager.class);
     ...
     */
    public static UiServerManager getInstance()
    {
    ...
    {
        public static final UiServerManager INSTANCE = new UiServerManager();
    }
    ...
    public void start()
    {
        ...
        _logger.info("Starting UI Web Sever on " + AppProperties.getInstance().getUiIpAddress() + ":"
                + AppProperties.getInstance().getUiIpPort() + "...");
        ...
        ExecutionHandler executionHandler = new ExecutionHandler(new OrderedMemoryAwareThreadPoolExecutor(
                appProperties.getUiTaskThreadPoolSize(), appProperties.getUiTaskThreadPoolMaxChannelMemorySize(),
                appProperties.getUiTaskThreadPoolMaxThreadMemorySize(),
                appProperties.getUiTaskThreadPoolKeepAliveSeconds(), TimeUnit.SECONDS));

        bootstrap.setPipelineFactory(new HttpServerPipelineFactory(executionHandler));

        // Bind and start to accept incoming connections.
        InetSocketAddress socket = new InetSocketAddress(AppProperties.getInstance().getUiIpAddress(), AppProperties
                .getInstance().getUiIpPort());
