...
* @MqManager@ is our wrapper for HornetQ.
...
There are 2 types of queues that is setup for a @Repository@.

* Write queues
** By convention, the physical HornetQ queues are named 'repository.XXXX.write'.
** These queues accept incoming log entries from one or more agents. Queued log entries are processed by one or more @RepositoryWorker@.
** Agents submitting log entries must provide credentials where the username is the repository name and the password is that specified in the repository meta-data.
** @RepositoryWorker@ consuming queued log entries must provide credentials of the trusted system user. 

* Read queues
** By convention, the physical HornetQ queues are named 'repository.XXXX.read'.
** These queues accept incoming log entries from one or more @RepositoryWorker@. Queued entries are processed by one or more agents. This is the reverse of a write queue.
** @RepositoryWorker@ submitting entries must provide credentials of the trusted system user.
** Agents consuming entries must provide credentials where the username is the repository name and the password is that specified in the repository meta-data.

HornetQ uses JAAS as the framework for authentication and authorization. Because we don't need to have real users accounts for agents or the system user (i.e. @users@), we use the following convention:
* The role required to read/write from a queue is the combination: '[username]~~~[password]'.
* When credentials are presented to HornetQ, our custom JAAS handler return the role in the above format.
* If the role matches then the agent is allowed to read from or write to the queue.
* The credentails for the trusted system user is managed by @AppProperties@. It is the single place from which all @RepositoryWorker@ can access the credentials.
* The trusted system user was used instead of separate credentials for each repository because @RepositoryWorker@ are created by our code so it can be trusted.


h2. 5. Web UI and API

We've chosen a client-server implementation for our user interface.

* Sever
** We use the Netty HTTP web server
** We create a HTTP REST API for use by our client (see below) or other clients as needs be. 
** @com.chililog.server.ui@ is the package containing all the server side code.

* Client
** We use Sproutcore framework to implement our client side user interface.
