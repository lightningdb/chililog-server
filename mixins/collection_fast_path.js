...
// NOTE FROM ENDASH: copied into my namespace to avoid conflicts with master
// while things are ironed out -- not taking credit
...
Endash.CollectionFastPath = {
  ...
  // Index translators, for 1 -> 1 transformation
  ...
  */
  wakePooledView: function(itemView, attrs) {
    ...
    // return
    var layer = itemView.get('layer'),
      layout = attrs.layout;
    
    // return

    // console.log(layer.id, layer.style.top, layout.top + "px")
    // console.log(layer.id, layer.style.width, layout.width + "px")
  ...
  */
  ...
  /**
    ...
    @index MUST BE translated through contentIndexforIndex before
      being passed in
  ...
  */
  itemViewForContentIndex: function(index) {
    ...
    var item = this.contentItemForIndex(index);
    if (!item) return null;
  ...
  */
  remapItemViews: function(nowShowing) {
    ...
    nowShowing.forEach(function(idx) {
      item = this.contentItemForIndex(idx);
    ...
    for (var idx = 0, len = mayExist.length; idx < len; idx++) {
      ...
      item = this.contentItemForIndex(newIdx);
  ...
  */
  reloadIfNeeded: function(nowShowing, scrollOnly) {
    ...
    // console.log('reload if needed')
    // console.log(this._itemsToAdd)
  ...
  */
  processUpdates: function(invalid) {
    var u = this._itemsToUpdate, item, view;
  ...
  */
  processAdds: function() {
    ...
    for (idx = 0; idx < len; idx++) {
      ...
      itemIdx = add[idx]; item = this.contentItemForIndex(itemIdx);
  ...
  */
  sendToDOMPool: function(view) {
    ...
    // console.log("length after sending", pool.get('length'))
  },
  ...
  */
  addItemView: function(exampleView, object, index) {
    ...
    // console.log(index, SC.guidFor(object), SC.guidFor(view))
  ...
  */
  touchScrollDidChange: function(left, top) {
    ...
    if (Date.now() - this._lastTouchScrollTime < 100) return;
    ...
    var r = this.computeNowShowing(cf)
